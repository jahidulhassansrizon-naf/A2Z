...
exports.sendOTP = async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ message: "এই ইমেইল দিয়ে অলরেডি অ্যাকাউন্ট আছে!" });
    }
    ...
    if (userRole === "student") {
      if (!roll) {
        return res
          .status(400)
          .json({ message: "অ্যাকাউন্টের জন্য ইউজার আইডি নম্বর আবশ্যক!" });
      }

      const existingRoll = await Student.findOne({ roll: Number(roll) });
      if (existingRoll) {
    ...
    if (role === "admin" && adminSecret !== "NAFIA") {
      return res
        .status(403)
        .json({ message: "ভুল Admin Secret Key! আপনি এডমিন হতে পারবেন না।" });
    }
    ...
    otpStore.set(email, {
      userData: { name, email, password, role: userRole, roll },
      otp: generatedOTP,
        ...
        <html>
          ...
          <body style="font-family: Arial, sans-serif; background-color: #f4f4f7; color: #51545e; margin: 0; padding: 20px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 8px; border: 1px solid #eaeaec; padding: 30px;">
              <tr>
                <td>
                  <h2 style="color: #333333; font-size: 20px; margin-top: 0;">Email Verification</h2>
                  <p style="color: #51545e; font-size: 14px;">Hello <b>${name}</b>,</p>
                  <p style="color: #51545e; font-size: 14px;">Use the verification code below to complete your registration on A to Z Platform:</p>
                  <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 6px; text-align: center; padding: 15px; margin: 20px 0;">
                    <span style="font-size: 32px; font-weight: bold; letter-spacing: 6px; color: #16a34a;">${generatedOTP}</span>
                  </div>
                  <p style="color: #6b7280; font-size: 12px;">This code is valid for 5 minutes. If you did not request this, please ignore this email.</p>
                  <hr style="border: none; border-top: 1px solid #eaeaec; margin: 20px 0;" />
                  <p style="color: #9ca3af; font-size: 11px; text-align: center;">© A to Z Management System. All rights reserved.</p>
                </td>
              </tr>
            </table>
          </body>
...
exports.verifyOTPAndRegister = async (req, res) => {
  try {
    ...
    if (Date.now() > record.expiresAt) {
      ...
      return res
        .status(400)
        .json({ message: "ওটিপির মেয়াদ শেষ হয়ে গেছে! আবার চেষ্টা করুন।" });
    }

    if (record.otp !== otp.trim()) {
      return res
        .status(400)
        .json({ message: "ভুল ওটিপি কোড! সঠিক কোডটি দিন।" });
    }
...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: user._id, role: user.role, roll: user.roll },
...
exports.getProfile = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: "ইউজার ডাটাবেসে পাওয়া যায়নি!" });
    }
